...
import freqStatAPI from '~/api/adminAPI/freqStatAPI';
function CoachesStat() {
    ...
    const handleStat = async () => {
        switch (filterSelected) {
            case 1:
                try {
                    const response2 = await freqStatAPI.getFreqMonthStat(monthFilterByMonth, year);
            ...
            case 2:
                try {
                    const response3 = await freqStatAPI.getFreqQuarterStat(quarterFilterByQuarter, year);
            ...
            default:
                try {
                    const response4 = await freqStatAPI.getFreqYearStat(year);
